...
const (
	...
	maxSourceReadBytes         = 8_388_609
	remoteTimeout              = time.Duration(30_000_000_000)
)
...
func run(arguments []string, fetch fetchFunc, writeFile writeFileFunc) error {
	...
	if strings.Join([]string{*countryOutput, *currencyOutput, *subdivisionOutput}, "") == "" {
...
func generateCountryData(validityReader, mappingsReader io.Reader) ([]byte, error) {
	...
	slices.SortFunc(records, func(left, right countryRecord) int {
		return cmp.Compare(left.alpha2, right.alpha2)
	})
...
func activeCurrencyRecord(entry currencyXMLEntry) (currencyRecord, error) {
	if !validCurrencyCode(entry.Code) {
		return currencyRecord{}, errors.New("invalid current currency entry")
	}
	if !validNumericCode(entry.Numeric) {
		return currencyRecord{}, errors.New("invalid current currency entry")
	}
	if entry.Name == "" {
		return currencyRecord{}, errors.New("invalid current currency entry")
...
func historicCurrencyRecord(entry currencyXMLEntry) (currencyRecord, error) {
	...
	if entry.Numeric != "" && !validNumericCode(entry.Numeric) {
		return currencyRecord{}, errors.New("invalid historic currency entry")
	}
	if entry.Name == "" || entry.Withdrawal == "" {
		return currencyRecord{}, errors.New("invalid historic currency entry")
	}
...
func validCurrencyCode(value string) bool {
	return validASCII(value, 3, 'A', 'Z')
}

func validNumericCode(value string) bool {
	return validASCII(value, 3, '0', '9')
}
...
func sameCharacterClass(left, right byte) bool {
	class := characterClass(left)
	return class != 0 && class == characterClass(right)
}
...
func validSubdivisionID(id string) bool {
	...
	if !validASCII(id[:2], 2, 'a', 'z') {
		return false
	}
	return validLowerAlphanumeric(id)
}

func validASCII(value string, length int, lower, upper byte) bool {
	if len(value) != length {
		return false
	}
	for _, character := range []byte(value) {
...
func characterClass(character byte) byte {
	switch {
